...
import { 
  User, Mail, Calendar, Target, Award, TrendingUp, 
  BarChart3, Clock, Edit3, X, ChevronRight, Loader2
} from 'lucide-react';
...
export default function ProfilePage() {
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
      ...
      <p className="text-slate-500 font-mono text-[10px] uppercase tracking-widest">Loading Records...</p>
    </div>
  );
    ...
    <div className="space-y-8 animate-in fade-in duration-700 pb-20 text-slate-100">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 text-sm font-light mt-1 tracking-tight underline decoration-emerald-500/20 underline-offset-4">Sistem Manajemen Identitas Sales v2.0</p>
        </div>
        <button 
          onClick={() => setIsModalOpen(true)}
          className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-white px-4 py-2 rounded-md text-xs font-bold border border-slate-700 transition-all shadow-sm"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* KARTU IDENTITAS */}
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 h-fit shadow-xl">
          <div className="flex flex-col items-center text-center">
            <div className="w-24 h-24 bg-emerald-600 rounded-2xl flex items-center justify-center mb-6 shadow-xl shadow-emerald-950/20 rotate-3 transition-transform hover:rotate-0 duration-500">
            ...
            <p className="text-sm text-emerald-500 font-bold mt-1 uppercase tracking-widest text-[10px]">{profile.role}</p>
          </div>
          ...
          <div className="mt-10 space-y-6">
            ...
            <div className="space-y-1">
              <p className="text-[10px] font-bold text-slate-600 uppercase tracking-[0.2em]">Corporate Email</p>
              <div className="flex items-center gap-3 text-slate-300">
            ...
            <div className="space-y-1">
              <p className="text-[10px] font-bold text-slate-600 uppercase tracking-[0.2em]">Join Date</p>
              <div className="flex items-center gap-3 text-slate-300">
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <StatBox icon={BarChart3} label="Total Records" value={profile.stats.leads_processed} colorClass="text-blue-400" />
            <StatBox icon={TrendingUp} label="Lead Accuracy" value={`${profile.stats.conversion_rate}%`} colorClass="text-emerald-400" />
            <StatBox icon={Clock} label="Days Active" value={`${profile.active_days}`} colorClass="text-orange-400" />
          </div>

          {/* TARGET CARD */}
          <div className="bg-slate-900 border border-slate-800 p-8 rounded-2xl shadow-lg border-l-4 border-l-rose-600">
            <div className="flex justify-between items-center mb-6">
              <div className="flex items-center gap-3">
                <Target className="text-rose-500" size={20} />
                <h4 className="text-sm font-bold text-white uppercase tracking-widest">Monthly Lead Goal</h4>
              </div>
            ...
            <div className="h-2 w-full bg-slate-800 rounded-full overflow-hidden mb-3">
              <div 
                className="h-full bg-rose-600 transition-all duration-1000 shadow-[0_0_10px_rgba(225,29,72,0.4)]"
                style={{ width: `${Math.min((profile.stats.current_progress / profile.monthly_target) * 100, 100)}%` }}
              />
            </div>
            <p className="text-[10px] text-slate-600 font-light italic">Target diakumulasi dari nasabah dengan status 'Interested' bulan ini.</p>
          </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
            ...
            <div className="divide-y divide-slate-800/50">
              {profile.recent_activities.length === 0 ? (
                <div className="p-10 text-center text-slate-600 text-xs italic">No recent activity detected in infrastructure.</div>
              ) : (
                profile.recent_activities.map((act: any, i: number) => (
                  <div key={i} className="p-4 hover:bg-white/[0.01] transition-all flex items-center justify-between group">
                    <div className="flex gap-4 items-center">
                      ...
                      <div>
                        <p className="text-xs text-slate-200 font-medium tracking-tight">{act.content}</p>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-slate-900 border border-slate-800 w-full max-w-md rounded-2xl shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center p-6 border-b border-slate-800">
              <h3 className="text-lg font-bold text-white tracking-tight">Update Sales Identity</h3>
            ...
            <form onSubmit={handleUpdate} className="p-6 space-y-6">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-slate-600 uppercase tracking-widest px-1">Professional Role</label>
                <input 
                  required
                  className="w-full bg-slate-950 border border-slate-800 rounded-lg py-2.5 px-4 text-sm text-slate-200 outline-none focus:border-emerald-500 transition-all"
                  value={formData.role}
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-slate-600 uppercase tracking-widest px-1">Monthly Target (Leads)</label>
                <input 
                  required
                  type="number"
                  className="w-full bg-slate-950 border border-slate-800 rounded-lg py-2.5 px-4 text-sm text-slate-200 outline-none focus:border-emerald-500 transition-all"
...
function StatBox({ icon: Icon, label, value, colorClass }: any) {
  return (
    <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl shadow-md border-t-2 border-t-slate-800 hover:border-t-emerald-500 transition-all">
